...
const RevenueChart = ({ data, color }) => (
  <ResponsiveContainer width="100%" height={180}>
    <BarChart data={data} margin={{ top:5, right:10, left:10, bottom:5 }}>
      ...
      <YAxis tickFormatter={v => `R$${v}M`} fontSize={10} tickLine={false} axisLine={false} />
...
// 3) Template único (Goldman) — você pode clonar essa função para outros estilos
const GoldmanTemplate = ({ data, logo }) => (
  <div style={{
    width:  '210mm', height: '297mm',
    padding: 16, boxSizing: 'border-box',
    backgroundColor: 'white', color: financeBlue,
    fontFamily: 'Arial, sans-serif', fontSize: 12
  }}>
    {/* Header */}
    <header style={{
      display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start',
      borderBottom: `2px solid ${financeGold}`, paddingBottom: 8, marginBottom: 16
    }}>
    ...
    <div style={{ display:'flex', gap:16 }}>
      ...
      <div style={{ flex:1 }}>
        ...
        <section style={{ marginBottom:16 }}>
          ...
          {data.companyImage
            ? <img src={data.companyImage} alt="Company" style={{
            ...
            : <div style={{
                ...
                justifyContent:'center', borderRadius:4, marginBottom:8
              }}>
          ...
          <div style={{ backgroundColor:financeLight, padding:8, borderRadius:4 }}>
            <h3 style={{
              margin:'0 0 4px 0', fontSize:10, fontWeight:500, color:'#666'
            }}>KEY STRENGTHS</h3>
            <ul style={{ margin:0, paddingLeft:16, listStyleType:'disc' }}>
              {data.companyStrengths.map((s,i)=><li key={i} style={{ fontSize:10 }}>{s}</li>)}
            </ul>
          </div>
        </section>
        {/* Transaction */}
        <section>
          ...
          <div style={{
            ...
            <ul style={{ margin:0, paddingLeft:16, listStyleType:'disc' }}>
              {data.transactionOptions.map((o,i)=><li key={i} style={{ fontSize:10 }}>{o}</li>)}
            </ul>
          </div>
        </section>
      </div>
...
// 4) App + editor + download + preview
export default function App() {
  const [data, setData] = useState(defaultData);
  const [logo, setLogo] = useState(null);
  const refDiv = useRef();

  const onChange = key => e => setData(d => ({ ...d, [key]: e.target.value }));
  ...
  const downloadPDF = async () => {
    const canvas = await html2canvas(refDiv.current, { scale:2 });
  ...
  return (
    <div style={{ padding:20, fontFamily:'sans-serif' }}>
      ...
      <div style={{
        display:'grid', gridTemplateColumns:'1fr 1fr', gap:16, margin:'16px 0'
      }}>
        {fields.map(f=>(
        ...
        <label style={{ display:'flex', flexDirection:'column' }}>
          Company Logo:
          <input type="file" accept="image/*" onChange={onUpload('companyImage')} style={{ marginTop:4 }} />
        </label>
      </div>
      ...
      <div style={{ border:'1px solid #ccc', overflow:'auto', textAlign:'center' }}>
        <div ref={refDiv} style={{ display:'inline-block', transform:'scale(0.7)', transformOrigin:'top center' }}>
          <GoldmanTemplate data={data} logo={data.companyImage} />
        </div>
      </div>
    </div>
  );
